...
def loadTestResult():
    l = []
    with open('data/knn_benchmark.csv') as file:
        lines = csv.reader(file)
        for line in lines:
...
def knnClassify(trainData, trainLabel, testData):
    ...
    saveResult(testLabel, 'data/sklearn_knn_Result.csv')
    return testLabel
...
def svcClassify(trainData, trainLabel, testData):
    ...
    saveResult(testLabel, 'data/sklearn_SVC_C=5.0_Result.csv')
    return testLabel
...
def GaussianNBClassify(trainData, trainLabel, testData):
    ...
    saveResult(testLabel, 'data/sklearn_GaussianNB_Result.csv')
    return testLabel
...
def MultinomialNBClassify(trainData, trainLabel, testData):
    ...
    nbClf.fit(trainData, ravel(trainLabel))
    testLabel = nbClf.predict(testData)
    saveResult(testLabel, 'data/sklearn_MultinomialNB_alpha=0.1_Result.csv')
    return testLabel
